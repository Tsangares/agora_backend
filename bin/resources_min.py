...
class TagResource_min(ModelResource):
    ...
    '''        resource_name='tag'
        ordering=["total_questions", "name", "slug", "id"]
        filtering={
            "slug": ALL,
            "name": ALL,
        }
    ...
    def apply_soring(self, obj_list, options=None):
        if options and "total_questions" == options.get('order_by', ''):
            return obj_list.order_by("slug")
        return obj_list.order_by("slug")
        return super (TagResource_min, self).apply_sorting(obj_list, options)
'''
